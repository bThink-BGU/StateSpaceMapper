...
public abstract class BPEngineTask implements Callable<BThreadSyncSnapshot>{
  ...
  BPEngineTask(BProgramSyncSnapshot aBpss, BThreadSyncSnapshot aBtss, Listener aListener) {
    aBpss.getBProgram().atilla();
    BPjsObjectCloner cloner = new BPjsObjectCloner(aBpss.getBProgram());
    bpss = BProgramSyncSnapshotCloner.clone(aBpss);
    btss = cloner.clone(aBtss);
    listener = aListener;
  }
