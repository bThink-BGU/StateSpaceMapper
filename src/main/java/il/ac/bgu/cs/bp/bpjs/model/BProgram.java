...
import il.ac.bgu.cs.bp.bpjs.bprogramio.BPjsObjectCloner;
...
public abstract class BProgram {
  ...
  public void atilla() {
    BPjsObjectCloner cloner = new BPjsObjectCloner(this);
    if(!recentlyEnqueuedExternalEvents.isEmpty())
      recentlyEnqueuedExternalEvents = cloner.clone(recentlyEnqueuedExternalEvents);
    if(!recentlyRegisteredBthreads.isEmpty())
      recentlyRegisteredBthreads = cloner.clone(recentlyRegisteredBthreads);
    if(!recentlyAddedForks.isEmpty())
      recentlyAddedForks = cloner.clone(recentlyAddedForks);
    programScope = cloner.clone(programScope);
  }
   ...
   */
  private BlockingQueue<BEvent> recentlyEnqueuedExternalEvents = new LinkedBlockingQueue<>();

  /**
   * BThreads added between syncs are added here.
   */
  private BlockingQueue<BThreadSyncSnapshot> recentlyRegisteredBthreads = new LinkedBlockingDeque<>();

  private BlockingQueue<ForkStatement> recentlyAddedForks = new LinkedBlockingDeque<>();
