...
public class DfsTraversalNode {
  ...
  protected DfsTraversalNode(BProgram bp, BProgramSyncSnapshot systemState, BEvent e) {
    BPjsObjectCloner cloner = new BPjsObjectCloner(bp);
    this.bp = bp;
    this.systemState = BProgramSyncSnapshotCloner.clone(systemState);
    this.lastEvent = e;

    if (bp != null) {
      bp.atilla();
      selectableEvents = (Set<BEvent>) cloner.clone(bp.getEventSelectionStrategy().selectableEvents(systemState));
