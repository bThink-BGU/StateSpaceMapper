...
let noam = function () {
  ...
  noam.util.HashTable = Map//require('structure.js').HashTable;
      ...
      */
      ...
      function simplify(tree, config) {
        ...
        while (appliedPattern !== null && (opts.numIterations === null || iterCount < opts.numIterations)) {
          ...
          appliedPattern = _simplify_iteration(treeClone, fsmCache, opts.useFsmPatterns, iterCount);
      ...
      function _simplify_iteration(tree, fsmCache, useFsmPatterns, iteration) {
        var pattern = null;
        var result = null;
...
function getRE(fsm) {
  let automata = noam.fsm.parseFsmFromString(fsm);
  let regex = noam.re.tree.toString(noam.fsm.toRegex(automata));
  return regex;
}
